...
try:
    while True:
        ...
        event_payload = {
            ...
            "telemetry": {
                "velocity_24h_count": (
                    random.randint(15, 50) if is_fraud else random.randint(1, 6)
                ),
            ...
            "agent_insights": {
                "graph_agent_output": (
                    f"Graph Cluster Node Match: Found edge connection to shared asset pool via {device}."
                    if is_fraud
                    else "Isolated single pathway node."
                ),
                "behavioral_agent_output": (
                    "High frequency automation keystroke script detected."
                    if is_fraud
                    else "Normal biometric pattern match."
                ),
            },
        }
