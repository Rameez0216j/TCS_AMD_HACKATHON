...
# --- HARDCODED CREDENTIALS (Bypasses .env path glitches) ---
NEO4J_URI = "neo4j+ssc://c8168987.databases.neo4j.io"
NEO4J_USER = "c8168987"
NEO4J_PASSWORD = (
    "Rw9dHdY1rV9k-to31oeiUs2wAHiWbTKU2X6-TtvpOYE"  # <-- Put your password here
)

# Absolute path for your Windows workspace layout
DATA_DIR = r"D:\TCS\TCS_AMD_HACKATHON\Data\synthetic_fraud_data"
BATCH_SIZE = 2000
...
def execute_batch(session, query, df, description):
...
def main():
    ...
    with driver.session() as session:
        ...
        # Sanction List
        df_sanction = pd.read_csv(os.path.join(DATA_DIR, "sanction_list.csv"))
        cypher_sanction = """
        UNWIND $batch AS row
        ...
        SET s.entity_name = row.entity_name,
            ...
            s.status = row.status
        """
        execute_batch(session, cypher_sanction, df_sanction, "Sanction Entities")
        ...
        """
        execute_batch(session, cypher_customers, df_customers, "Customers")

        # Devices
        df_devices = pd.read_csv(os.path.join(DATA_DIR, "devices.csv"))
        cypher_devices = """
        UNWIND $batch AS row
        ...
        """
        execute_batch(session, cypher_devices, df_devices, "Devices")

        # Beneficiaries
        df_beneficiaries = pd.read_csv(os.path.join(DATA_DIR, "beneficiaries.csv"))
        ...
        """
        execute_batch(session, cypher_beneficiaries, df_beneficiaries, "Beneficiaries")

        # Merchants
        df_merchants = pd.read_csv(os.path.join(DATA_DIR, "merchants.csv"))
        cypher_merchants = """
        UNWIND $batch AS row
        ...
        """
        execute_batch(session, cypher_merchants, df_merchants, "Merchants")
        ...
        df_behavior = pd.read_csv(os.path.join(DATA_DIR, "customer_behavior.csv"))
        cypher_behavior = """
        UNWIND $batch AS row
        ...
        df_analysis = pd.read_csv(
            os.path.join(DATA_DIR, "transaction_analysis_logs.csv")
        )
        cypher_analysis = """
        UNWIND $batch AS row
        MATCH (t:Transaction {transaction_id: row.transaction_id})
        ...
        """
        execute_batch(
            session, cypher_analysis, df_analysis, "Transaction AI Agent Analytics Logs"
        )
        ...
        df_cust_dev = pd.read_csv(os.path.join(DATA_DIR, "customer_devices.csv"))
        cypher_cust_dev = """
        UNWIND $batch AS row
        MATCH (c:Customer {customer_id: row.customer_id})
        ...
        df_cust_bene = pd.read_csv(os.path.join(DATA_DIR, "customer_beneficiaries.csv"))
        cypher_cust_bene = """
        UNWIND $batch AS row
        MATCH (c:Customer {customer_id: row.customer_id})
        ...
        """
        execute_batch(
            session, cypher_cust_bene, df_cust_bene, "Customer ↔ Beneficiary Mapping"
        )
